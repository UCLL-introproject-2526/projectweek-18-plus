...
# == Rendeir event ==
REINDEER_IMAGE = pygame.image.load(voorbeeldassetsreindeer_sleigh.png).convert_alpha()
REINDEER_IMAGE = pygame.transform.scale(REINDEER_IMAGE,(REINDEER_IMAGE.get_width()  8, REINDEER_IMAGE.get_height()  8))

# == Fonts ==
FONT_TITLE = pygame.font.Font(voorbeeldassetsfontsPressStart2P-Regular.ttf, 48)
FONT_TEXT = pygame.font.Font(voorbeeldassetsfontsMontserrat-Bold.ttf, 32)
FONT_SMALL = pygame.font.Font(voorbeeldassetsfontsMontserrat-Bold.ttf, 24)
...
try
    start_background = pygame.image.load(voorbeeldassetsbackground start.jpg).convert()
...
skins = [
    crop_surface(pygame.image.load(voorbeeldassetsSanta_Avatar.png).convert_alpha()),
    crop_surface(pygame.image.load(voorbeeldassetsSnowman.png).convert_alpha()),
    crop_surface(pygame.image.load(voorbeeldassetsElf .png).convert_alpha())
]
...
def show_front_screen(screen, start_background, highscore, last_score=None)
    ...
    while True
        ...
        screen.blit(title,(WIDTH2 - title.get_width()2, HEIGHT4))

        hs_text = FONT_TEXT.render(fHighscore {highscore}, True, (130, 5, 24))
        screen.blit(hs_text,(WIDTH2 - hs_text.get_width()2, HEIGHT4 + 90))

        instruction = FONT_SMALL.render(Press SPACE to start, True, (0, 0, 0))
        screen.blit(instruction,(WIDTH2 - instruction.get_width()2, HEIGHT4 + 200))

        # Last score
        if last_score is not None
            last_text = FONT_SMALL.render(fLast score {last_score}, True, (0, 0, 0))
            screen.blit(last_text, (WIDTH2 - last_text.get_width()2, HEIGHT4 + 140))


        # Skin select label
        skin_label = FONT_SMALL.render(Skin Select, True, (0, 0, 0))
        screen.blit(skin_label, (WIDTH2 - skin_label.get_width()2, HEIGHT4 + 260))

        # Skins (wheel selector)
        preview = pygame.transform.scale(skins[selected_index], PREVIEW_SIZE)
        screen.blit(preview,(WIDTH2 - PREVIEW_SIZE[0]2, HEIGHT4 + 320))
        ...
        screen.blit(preview,(CENTER_X - preview.get_width()  2, HEIGHT4 + 320))
...
def show_game_over(screen, score_value)
    draw_text_outline(FONT_TITLE, GAME OVER, (200,0,0), (0,0,0), WIDTH2, HEIGHT2)
    score_text = FONT_TEXT.render(fScore {score_value}, True, (0, 0, 0))
    score_rect = score_text.get_rect(center=(WIDTH2, HEIGHT2 + 60))
...
while running
    ...
    while game_active
        ...
        if paused
            ...
            screen.blit(pause_text, (WIDTH2 - pause_text.get_width()2, HEIGHT2))
            pygame.display.update()
        ...
        ammo_text = font.render(fAmmo {ammo}, True, (255, 255, 255))
